...
def plot_cell_depth():
    ...
    filename = os.path.join(
        paths["owncFig"], "cells/centri_neurons_histolog_170105.xlsx"
    )
    ...
    for a, b in zip(z1, labelled[kind + "_sig"].to_list()):
        if b == 1:
            ...
            z2.append("w")
    ax.bar(x, y, color=z2, edgecolor=z1, linewidth=3, alpha=0.6, width=0.8)
...
def barplot_cell_depth_all(spread="sect", amp="gain"):
    ...
    for ax, col, sig_col in zip(axes[0::2], cols, sig_cols):
        ...
        for a, b in zip(z1, labelled[sig_col].to_list()):
            if b == 1:
                ...
                z2.append("w")
        # ax.bar(x, y, color=z, alpha=0.6)
        ax.bar(x, y, color=z2, edgecolor=z1, linewidth=2, alpha=0.6, width=0.8)
        ax.set_xlabel("cell")
        ax.set_xlim(-0.5, len(x))
        lims = ax.get_xlim()
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "cellDepth.py:barplot_cell_depth_all",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
def dotplot_cell_depth_all(spread="sect", amp="gain"):
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "cellDepth.py:dotplot_cell_depth_all",
            ha="right",
            va="bottom",
            alpha=0.4,
        )

...
def plot_histo(df, num=8, kind='x'):
    ...
    colors = {'rndiso':'b', 'cpcross':'gold',
              'cpiso':'r', 'cfiso':'g'}
    fig, axs = plt.subplots(2, num//2, sharex=True, sharey=True,
                            tight_layout=True, figsize=(14, 8))
    axs = axs.flatten()
    if kind == 'x':
...
#            cols.append(col)
    ...
    for i, col in enumerate(cols):
        ...
        ax = axs[i]
        ax.hist(df[col], bins=15, density=True, color=color, label=label)#, sharex=ax0)
        ax.annotate(medStr, xy=(0.6, 0.6), xycoords='axes fraction')
        lims = ax.get_ylim()
        ax.vlines(0, lims[0], lims[1], alpha=0.3)
        ax.vlines(df[col].median(), lims[0], lims[1], color=color, alpha=0.4,
                  linewidth=2)
        ax.legend()
    # remove the frames
    for ax in axs:
    ...
    for i, key in enumerate(['Sector', 'Full']):
        ...
        for kind in kinds:
            if key in kind:
                ...
                ax.plot(df[x], df[y], 'o', label=label, alpha=0.6,
                        color=colors[label])
                ax.hlines(df[y].median(), xlims[0], xlims[1], alpha=0.3,
                          color=colors[label], linewidth=3)
                ax.vlines(df[x].median(), ylims[0], ylims[1], alpha=0.3,
                          color=colors[label], linewidth=3)
        ax.legend()
    for ax in axes:
...
def plot_rank(df, num=8, kind='_x'):
    ...
    colors = {'rndiso':'b', 'cpcross':'gold',
              'cpiso':'r', 'cfiso':'g'}
    fig, axs = plt.subplots(2, num//2, sharex=False, sharey=True,
                            tight_layout=True, figsize=(14, 8))
    axs = axs.flatten()
    if kind == '_x':
    ...
    for i, col in enumerate(cols):
        ...
        ax.hlines(sortedDf[col].median(), lims[0], lims[1],
...
def regress(df, out=False):
    ...
    stat_df = pd.DataFrame(index=['slope', 'intercept', 'r_value', 'p_value', 'std_err'])
    for item in list_stim(df):
        slope, intercept, r_value, p_value, std_err \
        = stats.linregress(df[[item + '_x', item + '_y']])
        adico = {}
        adico['slope'] = slope
...
def plot_scatter(df, stat_df=None, num=8, shareAxes=True):
    if shareAxes:
        ...
        sharex = False
        sharey = False
    fig, axs = plt.subplots(2, num//2, sharex=sharex, sharey=sharey,
                            tight_layout=True, figsize=(14, 8))
    axs = axs.flatten()
#    if kind == '_x':
#        title = 'latency advance (ms) : histogram + median value'
...
#        title = ''
    ...
    protocols = sorted([item for item in protocols if 'Sector' in item]) \
                + sorted([item for item in protocols if 'Full' in item])
    # define the colors
    colors = {'rndiso':'b', 'cpcross':'gold',
              'cpiso':'r', 'cfiso':'g'}
    # plot
    for i, proto in enumerate(protocols):
        ...
        label = proto
        ax = axs[i]
        ax.plot(df[proto +'_x'], df[proto +'_y'], 'o', color=color, label=label,
                alpha=0.5)
        ax.legend()
        # regression
...
def compute_regress_through_scipy(df):
    ...
    for protoc in protocols:
        slope, intercept, r_value, p_value, std_err \
        = stats.linregress(df[[protoc +'_x', protoc +'_y']])
...
plt.plot(cpisosecX, cpisosecY, markersize=15, marker=".", ls="",
...
fig2 = plt.figure(figsize=(12, 12))
plt.plot(cpisofullX, cpisofullY, markersize=15, marker=".", ls="",
         color='red', label='data points')
plt.plot(cpisofullX, intercept + slope*cpisofullX, 'b',
         label='fitted line, R^2 = 0.37')
...
fig2 = plt.figure(figsize=(12, 12))
plt.plot(cpisofull2X, cpisofull2Y, markersize=15, marker=".", ls="",
         color='red', label='data points')
plt.plot(cpisofull2X, intercept + slope*cpisofull2X, 'b',
         abel='fitted line, R^2 = 0.37')

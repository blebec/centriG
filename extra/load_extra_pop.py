...
def load_csv(filename):
    ...
    # layers
    layers = pd.read_csv(os.path.join(os.path.dirname(filename), "layers.csv"), sep=";")
    ...
    layersDict = {"2": "2/3", "3": "2/3", "5": "5/6", "6": "5/6"}
    ...
    while count > 0:
        ...
        for col in df.columns:
            if df[col].dtype != float:
                ...
                med = ser.median()
                mad = ser.mad()
                if (len(ser.loc[ser > (med + mult * mad)]) > 0) or len(
                    ser.loc[ser < (med - mult * mad)]
                ) > 0:
    ...
    for col in stats_df.columns:
        print(
            "mean {:.2f} ±({:.2f})\t sig {:.2f} ±({:.2f}) \t {}".format(
                statsdf.loc["mean", [col]][0],
                statsdf.loc["std", [col]][0],
                statssigdf.loc["mean", [col]][0],
                statssigdf.loc["std", [col]][0],
                col,
            )
        )

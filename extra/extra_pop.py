...
if csvLoad:
    ...
    # xcel
    params = {"0": "1319_CXLEFT", "1": "2019_CXRIGHT"}
    sheet = "1"
...
def check_diff():
    ...
    for num in range(2):
        ...
        print(
            "xcel {:.1f}±{:.1f}, csv {:.1f}±{:.1f}".format(
                meanxcel, stdxcel, meancsv, stdcsv
            )
        )
...
def plot_boxplots(datadf, removemax=True, params=params, mes=None):
    ...
    """
    colordict = {"0c": "tab:grey", "s0": "tab:green", "sc": "tab:red", "00": "tab:blue"}
    ...
    for i, dats in enumerate(conds):
        ...
        for col in dats:
            if len(datadf[col].dropna()) == 0:
                dats.remove(col)
        bp = ax.boxplot(
            datadf[dats].dropna(), meanline=True, showmeans=True, patch_artist=True
        )
        # add color code
        ...
        for j, n in enumerate(y, 1):
            ax.text(x=j, y=0, s=str(n), ha="center", va="center", color="tab:grey")
        ...
        ax.axhline(
            med + 2 * mad, color="tab:blue", linewidth=2, linestyle=":", alpha=0.7
        )
        ax.axhline(
            med - 2 * mad, color="tab:blue", linewidth=2, linestyle=":", alpha=0.7
        )
...
def filter_effect(filename=file_name, removemax=True, params=params, mes="on"):
    ...
    fig, axes = plt.subplots(nrows=2, ncols=3, figsize=(14, 16), sharey=True)
    axes = axes.flatten()
    fig.suptitle(mes)
    ...
    for i, k in enumerate(dfs):
        ...
        for j, col in enumerate(cols):
            ...
            if len(df[col].dropna()) == 0:
                cols.remove(col)
        bp = ax.boxplot(
            df[cols].dropna(), meanline=True, showmeans=True, patch_artist=True
        )
        # add color code
    ...
    for mes in ["on", "hh", "int"]:
        ...
        if anot:
            ...
            fig.text(
                0.99,
                0.01,
                "extra/extra_pop/plot_all_histo",
                ha="right",
                va="bottom",
                alpha=0.4,
            )
...
#%%
def plot_on_histo(
    datadf, removemax=True, sheet=sheet, diff=False, shift=False, hh=False
):
    ...
    fig, axes = plt.subplots(
        nrows=2, ncols=3, figsize=(15, 12), sharex=True, sharey=True
    )
    if diff:
...
def plot_histo_3conds(
    datadf, removemax=True, sheet=sheet, diff=False, shift=False, hh=False, mes="on"
):
    ...
    for i, col in enumerate(dats):
        ...
        if diff:
            ...
            ax.axvline(med, color=colordict[col.split("_")[2]], alpha=0.5, linewidth=2)
            print(ser)
        else:
            ser = df[col].dropna()
            # ax.axvspan(med - mad, med+mad, color='tab:blue', alpha=0.3)
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "extra/extra_pop/plot_histo_3conds",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
def plot_on_scatter(
    ...
    colordict = {"0c": "tab:blue", "s0": "tab:green", "sc": "tab:red", "00": "tab:grey"}
    ...
    fig, axes = plt.subplots(
        nrows=ceil(len(ons) / 3), ncols=3, figsize=(15, 24), sharex=True, sharey=True
    )
    ...
    for i, col in enumerate(ons):
        ...
        # layers
        ax.axhspan(
            bylayer.loc["4"].dmin, bylayer.loc["4"].dmax, color="tab:grey", alpha=0.2
        )
        if diff:
            # one ref
        ...
        for j in range(len(bylayer)):
            ymin, ymax, med, mad = bylayer.iloc[j][["dmin", "dmax", "meds", "mads"]]
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "extra/extra_pop/plot_on_scatter",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
    ...
    fig.tight_layout()
    fig.subplots_adjust(
        top=0.967, bottom=0.058, left=0.038, right=0.995, hspace=0.5, wspace=0.099
    )
    return fig
...
for diff in what:
    for shift in what:
        for hh in what:
            fig = plot_on_scatter(
                data_df, diff=diff, shift=shift, hh=hh, removemax=True
            )
...
def plot_latencies(
    datadf, lat_mini=10, lat_maxi=80, sheet=sheet, xcel=False, layersloc=layers_loc
):
    """
    plot the latencies
    input :
    ...
    for k, dcols in enumerate([d0_cols, d1_cols]):
        ...
        for i, col in enumerate(dcols[1:]):  # drop layers column
            ...
            ax.plot(x, y, ".", color=colors[i + k], markersize=10, alpha=0.5, label=col)
            ...
            for j in range(len(bylayer)):
                ...
                if not np.isnan(med):
                    ax.vlines(
                        med, ymin, ymax, color=colors[i + k], alpha=0.5, linewidth=3
                    )
            ...
            txt = "med : {:.0f}±{:02.0f} ({:.0f}, {:.0f}, {:.0f})".format(
                df[col].median(),
                df[col].mad(),
                bylayer.meds.values[0],
                bylayer.meds.values[1],
            ...
            if len(xvals) > 0:
                ...
                if k == 0:
                    vax.plot(
                        x_kde, kde(x_kde), color=colors[i + k], alpha=0.6, linewidth=2
                    )
                else:
                    vax.plot(
                        x_kde,
            ...
            hax.barh(
                y=y, width=width, left=left, height=0.3, color=colors[i + k], alpha=0.4
            )
            # med
            bylayer["width"] = 1
            bylayer["left"] = bylayer.meds - 0.5
            y, left = bylayer[["y", "left"]].dropna().T.values.tolist()
            hax.barh(y=y, width=1, left=left, height=0.3, color=colors[i + k], alpha=1)
    ...
    bylayer["left"] = bylayer.meds - bylayer.mads
    y, width, left = bylayer[["y", "width", "left"]].dropna().T.values.tolist()
    h0.barh(y=y, width=width, left=left, height=0.3, color="tab:grey", alpha=0.4)
    # med
    bylayer["width"] = 1
    bylayer["left"] = bylayer.meds - 0.5
    y, left = bylayer[["y", "left"]].dropna().T.values.tolist()
    h0.barh(y=y, width=1, left=left, height=0.3, color="tab:grey", alpha=1)
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "extra/extra_pop/plot_latencies",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
        txt = "{} , data <-> sheet {} : {}".format(
            date, "_".join(str(sheet).split("_")[:3]), params.get(sheet, "")
        )
        fig.text(0.01, 0.01, txt, ha="left", va="bottom", alpha=0.4)
    fig.tight_layout()
...
def plot_latencies_bis(
    datadf, lat_mini=10, lat_maxi=80, sheet=sheet, layersloc=layers_loc, xcel=False
):
    """
    plot the latencies
    input :
    ...
    for i, col in enumerate(cols[1:]):
        ...
        ax.plot(y, x, ".", color=colors[i], alpha=0.7, ms=10, label=label)
        ##########
        bylayer["meds"] = df.groupby("layers")[col].median()
        bylayer["mads"] = df.groupby("layers")[col].mad()
        for j in range(len(bylayer)):
            ...
            # test if values are present
            if not np.isnan(med):
                ax.vlines(med, ymin, ymax, color=colors[i], alpha=0.5, linewidth=3)
        txt = "med : {:.0f}±{:02.0f} ({:.0f}, {:.0f}, {:.0f})".format(
            df[col].median(),
            df[col].mad(),
            bylayer.meds.values[0],
    ...
    #     ax.axhline(d, color='tab:grey', alpha=0.5)
    ax.axhspan(
        bylayer.dmin.values[1], bylayer.dmin.values[2], color="tab:grey", alpha=0.4
    )
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "extra/extra_pop/plot_latencies_bis",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
fig = plot_latencies_bis(data_df, lat_mini=0, lat_maxi=80, sheet=sheet, xcel=False)
...
def plot_d1_d0_low(datadf, sheet, param, high=False):
    ...
    def add_conf_interval(ax, data, kind="med"):
        ...
        ax.text(ax.get_xlim()[1], med, txt, va="bottom", ha="right", color="tab:blue")
        ax.text(
            ax.get_xlim()[1],
            med + 2 * mad,
            "med+2*mad",
    ...
    def add_regress(ax, data):
        ...
        fxs = (
            intercept + slope * data[cols[0]].min(),
    ...
    cols = subdf.columns
    ax.scatter(
        subdf[cols[0]], subdf[cols[1]], marker="o", s=65, alpha=0.8, color="tab:blue"
    )
    med, mad = subdf[cols[0]].median(), subdf[cols[0]].mad()
    ax.axvspan(med - mad, med + mad, color="tab:blue", alpha=0.3)
    txt = "{:.0f}±{:.0f}".format(med, mad)
    ax.text(med, subdf.max().max(), txt, va="top", ha="center", color="tab:blue")
    med = subdf[cols[1]].median()
    mad = subdf[cols[1]].mad()
    ax.axhspan(med - mad, med + mad, color="tab:blue", alpha=0.3)
    txt = "{:.0f}±{:.0f}".format(med, mad)
    ax.text(subdf.max().max(), med, txt, va="center", ha="right", color="tab:blue")
    ...
    if high:
        txt = "{} - {}".format(
            "_".join(select[2].split("_")[2:]), "_".join(select[1].split("_")[2:])
        )
    ax.set_ylabel(txt)
    ...
    ax.axvspan(med - mad, med + mad, color="tab:blue", alpha=0.3)
    txt = "{:.0f}±{:.0f}".format(med, mad)
    ax.text(med, ax.get_ylim()[1], txt, va="top", ha="center", color="tab:blue")
    add_conf_interval(ax, subdf[cols[1]])
    # labels
    txt = "{}".format(cols[1])
    if high:
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "extra/extra_pop/plot_d1_d0_low",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
def plot_d1_d2_high(datadf, sheet, shift=True):
    ...
    isi_shift = isi.get(sheet, 0)
    select = ["layers", "latOn_d0_(c)", "latOn_d1_s_(25°/s)", "latOn_d0_(s+c)_150°/s"]
    select = ["layers", "on_d0_0c_25", "on_d1_s0_25", "on_d0_sc_150"]
    df = datadf[select].copy()
    for col in df.columns[1:]:
        df.loc[df[col] == 100, col] = np.nan
        df.loc[df[col] < 1, col] = np.nan
    ...
    ax.plot(
        df.index, df[select[2]] - df[select[3]], "o", alpha=0.8, ms=10, color="tab:blue"
    )
    med = (df[select[2]] - df[select[3]]).median()
    mad = (df[select[2]] - df[select[3]]).mad()
    txt = "{:.0f} ± {:.0f} msec".format(med, mad)
    ax.text(
        1, 0.6, txt, va="bottom", ha="right", transform=ax.transAxes, color="tab:blue"
    )

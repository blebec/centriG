...
def plot_2_indMoySigNsig(data, colsdict, stdcolors, fill=True, anot=False):
    ...
    for i, col in enumerate(cols):
        ax.plot(data[col], color=colors[i], alpha=alphas[i], label=col)
    # start point
    x = 41.5
    ...
    for i, col in enumerate(cols[::-1]):
        ax.fill_between(
            data.index, data[col], color=colors[::-1][i], alpha=0.5, label=col
        )
        ax.plot(data[col], color=inv_colors[i], alpha=1, label=col)  # , linewidth=1)
    ...
    ax.annotate("n=37", xy=(0.2, 0.8), xycoords="axes fraction", ha="center")
    # popVmSig
    ...
    ax.annotate("n=27", xy=(0.2, 0.8), xycoords="axes fraction", ha="center")
    # pop spike
    cols = colsdict["popSpk"]
    ax = spkaxes[1]
    for i, col in enumerate(cols[::-1]):
        ax.plot(df[col], color=inv_colors[i], alpha=1, label=col)  # , linewidth=1)
        # ax.fill_between(df.index, df[col],
        #                 color=inv_colors[i], alpha=0.5, label=col)
    ax.annotate("n=20", xy=(0.2, 0.8), xycoords="axes fraction", ha="center")
    ...
    for i, col in enumerate(cols[2:]):
        ...
        #     ax.plot(df[col[j]], color=colors[i],
        #             alpha=1, label=col, linewidth=0.5)
    ax.annotate("n=15", xy=(0.2, 0.8), xycoords="axes fraction", ha="center")

    # labels
    for ax in axes:
        for spine in ["top", "right"]:
            ax.spines[spine].set_visible(False)
    ylabels = ["Membrane potential (mV)", "Normalized membrane potential", "", ""]
    for i, ax in enumerate(vmaxes):
        ax.axes.get_xaxis().set_visible(False)
        ax.spines["bottom"].set_visible(False)
        ax.set_ylabel(ylabels[i])
    ylabels = ["Firing rate (spikes/s)", "Normalized firing rate", "", ""]
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "old_figs.py:old_figs.plot_2_indMoySigNsig",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
def plot_2_indMoy(data, colsdict, stdcolors, anot=False):
    ...
    ax = spkaxes[0]
    for i, col in enumerate(cols[::-1]):
        ax.plot(data[col], color=colors[::-1][i], alpha=1, label=col, linewidth=1)
        ax.fill_between(
            data.index, data[col], color=colors[::-1][i], alpha=0.5, label=col
        )
    # plots pop (column 1-3)
    ...
    ax = spkaxes[1]
    for i, col in enumerate(cols[::-1]):
        ax.plot(df[col], color=colors[::-1][i], alpha=1, label=col)  # , linewidth=1)
        # ax.fill_between(df.index, df[col],
    ...
    if anot:
        date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        fig.text(
            0.99, 0.01, "old_figs.py:plot_2_indMoy", ha="right", va="bottom", alpha=0.4
        )
        fig.text(0.01, 0.01, date, ha="left", va="bottom", alpha=0.4)
    return fig
...
def plot_2_sigNsig(data, colsdict, stdcolors, fill=True, anot=False):
    ...
    if anot:
        date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        fig.text(
            0.99, 0.01, "old_figs.py:plot_2_sigNsig", ha="right", va="bottom", alpha=0.4
        )
        fig.text(0.01, 0.01, date, ha="left", va="bottom", alpha=0.4)
    return fig
...
def plot_3_signonsig(stdcolors, anot=False, substract=False):
    ...
    filenames = dict(
        pop=os.path.join(dirname, "fig3.xlsx"),
    ...
    # samplesize
    cellnumbers = {"pop": 37, "sig": 10, "nsig": 27}
    colors = [
        "k",
        stdcolors["red"],
        stdcolors["green"],
    ...
    for i, kind in enumerate(["sig", "nsig"]):
        ...
        for j, col in enumerate(cols):
            ax.plot(df[col], color=colors[j], alpha=alphas[j], label=col, linewidth=2)
            ax.annotate(
                "n=" + str(ncells), xy=(0.1, 0.8), xycoords="axes fraction", ha="center"
            )
        if substract:
            leg = ax.legend(
                loc="upper right", markerscale=None, frameon=False, handlelength=0
            )
        else:
            leg = ax.legend(
                loc="lower right", markerscale=None, frameon=False, handlelength=0
            )
        for line, text in zip(leg.get_lines(), leg.get_texts()):
            text.set_color(line.get_color())
        # blue point
        ax.plot(
            0, df.loc[0]["CNT-ONLY"], "o", color=stdcolors["blue"], ms=10, alpha=0.8
        )

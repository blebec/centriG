...
def plot_figure3(datadf, stdcolors, **kwargs):
    ...
    for i, col in enumerate(cols):
        ax.plot(df[col], color=colors[i], alpha=alphas[i], label=labels[i], linewidth=2)
    # bluePoint
    x = 0
    y = df.loc[0][df.columns[0]]
    vspread = 0.02  # vertical spread for realign location
    ...
    if addinsert:
        axins = fig.add_axes([0.5, 0.21, 0.42, 0.25], facecolor="w", alpha=0.2)
        for i, col in enumerate(cols):
            if i in [0, 4]:
                axins.plot(
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "pop_traces.py:plot_figure3(" + kind + ")",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
    ...
    for kind in ["pop", "sig", "nsig"]:
        for rec in ["vm", "spk"]:
            for spread in ["sect", "full"]:
                ...
                df, f = ltra.load_intra_mean_traces(
                    paths, kind=kind, age=age, rec=rec, spread=spread
                )
...
def plot_trace2x2(dflist, stdcolors, **kwargs):
    ...
    fig, axes = plt.subplots(
        nrows=2, ncols=2, figsize=(17.6, 17.6), sharex=True, sharey=True
    )
    axes = axes.flatten()
    # fig.suptitle(titles[kind], alpha=0.4)
    # if anot:
    ...
    for i, ax in enumerate(axes):
        ...
        if addinsert:
            # insert subplot inside this one (broader x axis)
            axins = ax.inset_axes([0.4, 0.11, 0.42, 0.25], facecolor="w", alpha=0.2)
            for i, col in enumerate(cols):
                if i in [0, 4]:
                    axins.plot(
                        df[col],
...
def plot_trace_1x2(stdcolors, **kwargs):
    ...
    files = dict(
        sect="union_idx_fill_sig_sector.xlsx", full="union_idx_fill_sig_full.xlsx"
    )
    filename = os.path.join(
        paths["owncFig"], "data", "averageTraces", "controlsFig", files[spread]
    )
    ...
    fig, axes = plt.subplots(
        nrows=2, ncols=1, figsize=(8.5, 8.5), sharex=True, sharey=False
    )
    axes = axes.flatten()
    fig.suptitle(title, alpha=0.4)
    ...
    # adjust
    y_labels = ["Normalized Vm", "Normalized Firing Rate"]
    ...
    for i, ax in enumerate(axes):
        ...
        ax.text(
            0.1, 0.8, txt, ha="left", va="center", transform=ax.transAxes, size="large"
        )

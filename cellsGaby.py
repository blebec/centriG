...
def print_files():
    ...
    files = [
        st for st in os.listdir(dirname) if not os.path.isdir(os.path.join(dirname, st))
    ]
    print("=" * 20)
...
def load_gaby():
    ...
    # lowercase
    df.gene_file_old = df.gene_file_old.apply(lambda st: str(st).lower())
    # fill names
...
def select_corrd(df, key="revcor", printcells=False):
    ...
    print("=" * 20)
    print(
        "for key={} we have {} cells ({} records)".format(
...
def plot_xy_distri(keydf, key="revcor", bins=10, pix=False):
    ...
    axes = axes.flatten()
    txt = key + " ( {}cells {}records )".format(len(keydf.cell.unique()), len(keydf))
    fig.suptitle(txt)
    ...
    q25, q75 = keydf[item + "x"].quantile([0.25, 0.75])
    ax.axvspan(q25, q75, color="tab:grey", alpha=0.3)
    q25, q75 = keydf[item + "y"].quantile([0.25, 0.75])
    ...
    q25, q75 = keydf[item + "l"].quantile([0.25, 0.75])
    ax.axhspan(q25, q75, color="tab:grey", alpha=0.3)
    ...
    width = round((np.max(meds) - np.min(meds)) / bins * 0.9)
    ...
    for neur in keydf.cell.unique():
        ...
        ax.plot(x, marker="o", alpha=0.7)
    q25, q75 = keydf[item + "theta"].quantile([0.25, 0.75])
    ax.axhspan(q25, q75, color="tab:grey", alpha=0.3)
    ...
    for neur in keydf.cell.unique():
        ...
        ax.plot(x, marker="o", alpha=0.7)
    q25, q75 = keydf[item + "w"].quantile([0.25, 0.75])
    ax.axhspan(q25, q75, color="tab:grey", alpha=0.3)

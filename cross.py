...
def plot_sorted_responses(
    df_left, df_right, mes="", overlap=True, left_sig=True, right_sig=True
):
    """
    ...
    left_axes.append(ax)
    for i in range(1, 4):
        left_axes.append(fig.add_subplot(gs[i, 0], sharey=ax, sharex=ax))
    # right
    right_axes = []
    ax = fig.add_subplot(gs[0, 1])
    right_axes.append(ax)
    for i in range(1, 4):
        right_axes.append(fig.add_subplot(gs[i, 1], sharey=ax, sharex=ax))
    ...
    if left_sig:
        ...
        for i, name in enumerate(traces):
            ...
            sig_name = name + "_sig"
            select = df_left[[name, sig_name]].sort_values(
            ...
            ax.bar(
                x, select, color=bar_colors, edgecolor=edge_color, alpha=0.8, width=0.8
            )
            if i == 0:
    ...
    if right_sig:
        ...
        for i, name in enumerate(traces):
            ...
            color_dic = {0: "w", 1: edge_color}
            sig_name = name + "_sig"
            select = df_right[[name, sig_name]].sort_values(
                by=[name, sig_name], ascending=False
            )
            bar_colors = [color_dic[x] for x in select[sig_name]]
            ax = right_axes[i]
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "cross.py:plot_sorted_responses",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
def scatter_lat_gain(df_left, df_right, mes=""):
    ...
    dark_colors = [
        std_colors["dark_red"],
        std_colors["dark_green"],
        std_colors["dark_yellow"],
    ...
    if anot:
        date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        fig.text(
            0.99, 0.01, "cross.py:scatter_lat_gain", ha="right", va="bottom", alpha=0.4
        )
        fig.text(0.01, 0.01, date, ha="left", va="bottom", alpha=0.4)
    return fig
...
def histo_lat_gain(df_left, df_right, mes=""):
    ...
    title = "responses" + " (" + mes + " " + spread + ")"
    anotx = "Cell rank"
    anoty = [df_left.columns[0].split("_")[1], df_right.columns[0].split("_")[1]]
    # anoty = ['Relative peak advance(ms)', 'Relative peak amplitude']
    #          #(fraction of Center-only response)']
    # plot
    fig = plt.figure(figsize=(8, 8))
    ...
    for i, name in enumerate(df_left.columns):
        ...
        # ax.set_title(name)
        ax.hist(
            df_left[name], bins=15, color=colors[i], edgecolor=dark_colors[i], alpha=0.8
        )
    ...
    for i, name in enumerate(df_left.columns):
        ...
        ax.vlines(
            med, lims[0], lims[1], color=dark_colors[i], linewidth=2, linestyle=":"
        )
        tx = "med=" + str(round(med, 2))
        ax.text(
            0.65, 0.8, tx, horizontalalignment="left", transform=ax.transAxes, alpha=0.5
        )
        # right
    for i, name in enumerate(df_right.columns):
        ax = right_axes[i]
    ...
    if anot:
        date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        fig.text(
            0.99, 0.01, "cross.py:histo_lat_gain", ha="right", va="bottom", alpha=0.4
        )
        fig.text(0.01, 0.01, date, ha="left", va="bottom", alpha=0.4)
    return fig

...
speed_info_df = pd.read_excel(os.path.join(paths["traces"], "neuron_props_speed.xlsx"))
...
def load_all_traces(info_df, folder="vm_all"):
    stims = [
        item
        for item in os.listdir(os.path.join(paths["traces"], folder))
        if item[0] != "."
    ]
    # remove dir
    for stim in stims:
...
def testPlot(dico, sig_cells):
    ...
    ax.fill_between(
        x=data_cond.index, y1=(middle - errs), y2=(middle + errs), color="r", alpha=0.3
    )
    middle = data_ref.mean(axis=1)
    errs = data_ref.std(axis=1)
    ax.plot(middle, "k-", alpha=0.6, label=cond)
    ax.fill_between(
        x=data_ref.index, y1=(middle - errs), y2=(middle + errs), color="k", alpha=0.2
    )
    # med & mad
    ax = axes[3]
    ...
    ax.plot(diff, "-g", linewidth=2, alpha=0.8, label="diff")
    x = diff.index.values
...
def simple_plot(cond, cell_list, df_list, lag_list, traces=True):
    ...
    fig, axes = plt.subplots(
        nrows=nrows, ncols=ncols, sharex=True, sharey=True, figsize=(16, 9)
    )
    axes = axes.T.flatten()
    ...
    for i, df in enumerate(df_list):
        ...
        if traces:
            ...
            ax.plot(df.surround_then_center, "-", color=color, alpha=0.8, label="cp")
        ...
        ax.plot(lag_list[i], df.loc[lag_list[i], ["surround_only"]], "+g", alpha=0.8)
    ...
    if anot:
        ...
        fig.text(
            0.99, 0.01, "vmPlot.py:simple_plot", ha="right", va="bottom", alpha=0.4
        )
...
def extract_data(cells, cond="cpisosec", sort_by="lag"):
    ...
    if sort_by == "amp":
        # sort by amp
        s_amps, s_lags, s_cells, s_dfs = sort_together(
            [amps, lags, cells, dfs], reverse=True
        )
    else:
        # sort by lag

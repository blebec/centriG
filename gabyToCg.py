...
def extract_data(filename):
    ...
    for label, val in zip(sorted(list(labels)),
                          [gs, lines, polylines]):
...
def plot_cgGabyVersion(datadf):
    ...
    for i, cell in enumerate(cells):
        ...
        for i, col in enumerate(cols):
            ax.plot(df[col], linestyle=style[i], linewidth=linewidth[i],
                    color=colors[i], 
                    alpha=alpha[i], label=labels[i])
    ...
    if anot:
        ...
        fig.text(0.99, 0.01, 'GabyToCg:plot_cgGabyVersion',

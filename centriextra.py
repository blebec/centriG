...
def load_data_file(filename, dt):
    """
    ...
    for key in data:
        if key not in ["__header__", "__version__", "__globals__", "DataFile"]:
            reformated_data = {
                "t": np.arange(
                    len(np.array(data["PSTH_STIM_1_ELEC_1_TRIAL_1"]).flatten())
                )
                * dt
            }
            ...
            for cond in conditions:
                ...
                for elec in electrodes:
                    ...
                    for trial in range(31):  # trials:
                        if cond == 10:
                            if (
                                "PSTH_STIM_%d_ELEC_%d_TRIAL_%d" % (cond, elec, trial)
                                in data
                            ):
                                reformated_data[cond, elec].append(
                                    np.array(
                                        data[
                                            "PSTH_STIM_%d_ELEC_%d_TRIAL_%d"
                                            % (cond, elec, trial)
                                        ]
                                    ).flatten()
                                )

...
def plot_figure2_proposal(datadf, colsdict, anot=False, age="new"):
    ...
    alphas = [0.8, 0.8]
    vspread = 0.06  # vertical spread for time alignement
    ...
    for i, col in enumerate(cols[::-1]):
        ax.plot(datadf[col], color=colors[::-1][i], alpha=1, label=col, linewidth=1)
        ax.fill_between(
            datadf.index, datadf[col], color=colors[::-1][i], alpha=0.5, label=col
        )
    # response point
    ...
    ax.annotate("n=22", xy=(0.2, 0.8), xycoords="axes fraction", ha="center")
    # response point
    x = 0
    y = datadf[cols[0]].loc[x]
    # ax.plot(x, y, 'o', color='tab:gray', ms=10, alpha=0.8)
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "fig_proposal.py:plot_figure2_proposal",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
def plot_2B_bis(stdcolors, anot=False, age="new"):
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "fig_proposal.py:plot_figure2B_bis",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
def align_center(adf, showPlot=False):
    ...
    if showPlot:
        ...
        ax.plot(
            ref.shift(int(10 * adv)), ":k", alpha=0.5, label="ref_corr", linewidth=2
        )
        ax.plot(cp - ref, "-b", alpha=0.5, label="cp - ref")
        ax.plot(cp - ref_corr, ":b", alpha=0.5, label="cp - ref_corr", linewidth=2)
        ax.legend()
        fig.tight_layout()
        for spine in ["top", "right"]:
            ax.spines[spine].set_visible(False)
...
def plot_indFill_bis(data, stdcolors, linear=True, substract=False):
    ...
    for i, col in enumerate(cols):  # omit centrerOnly
        ...
        # linear predictor
            ...
            ax.plot(df.loc[-120:200, [col]], color=colors[i], alpha=1, label=col)
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "fig_proposal.py:plot_indFill_bis",
            ha="right",
            va="bottom",
            alpha=0.4,
        )
...
def plot_pop_fill_bis(data, stdcolors=std_colors):
    ...
    colors = [
        std_colors[item]
        for item in ["k", "red", "dark_green", "blue_violet", "red", "blue_violet"]
    ]
    ...
    ax = fig.add_subplot(111)
    ax.plot(
        df.popfillVmscpIsoPDlp, ":r", alpha=1, linewidth=2, label="sThenCent - cent"
    )
    # surroundOnly
    ax.plot(
        df.surroundOnlysdUp,
        color=stdcolors["dark_green"],
        alpha=0.7,
        label="surroundOnly",
    )
    ax.fill_between(
    ...
    ax.axvline(x0, color="tab:blue", linestyle=":", alpha=0.8, linewidth=2)
    # end
    x2 = 124.6
    y = df["centerOnly"].loc[x2]
    ax.axvline(x2, color="tab:blue", linestyle=":", alpha=0.8, linewidth=2)
    ...
    if anot:
        ...
        fig.text(
            0.99,
            0.01,
            "fig_proposal.py:plot_pop_fill_bis",
            ha="right",
            va="bottom",
            alpha=0.4,
        )

...
savePath = os.path.join(
    paths["owncFig"], "pythonPreview", "proposal", "5_enerPeakOrGain"
)
...
g = g.map_diag(plt.hist, stacked=True, edgecolor="k", alpha=0.6)
...
def do_lmplot_plot(df, kind="vm", spread="sect"):
    ...
    g1 = sns.lmplot(
        x="time50", y="gain50", hue="stim", hue_order=hue_order, data=sub_df
    )
    g2 = sns.lmplot(x="time50", y="engy", hue="stim", hue_order=hue_order, data=sub_df)
    title = kind + "  " + spread
    for g in [g1, g2]:
        g.fig.suptitle(title)
        g.fig.tight_layout()
...
save = False
savePath = os.path.join(paths["owncFig"], "pythonPreview", "proposal", "enerPeakOrGain")
mes = ["Gain", "Engy"]
for spread in ["sect", "full"]:
    for kind in ["vm", "spk"]:

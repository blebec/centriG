...
for kind in ['vm', 'spk'][::-1]:
    ...
    #conds
    conds = list(set([item.split('_')[0] for item in mg_df.columns]))
    params = list(set([item.split('_')[1] for item in mg_df.columns]))

    if len(res_df.columns) < 1:
        cols = params.copy().append('kind')
        res_df = pd.DataFrame(columns=cols)

    for cond in conds:
        afilter = [('_').join([cond, item]) for item in params]    
        df = mg_df[afilter].reset_index(drop=True)
        df.columns = params
        df['kind'] = kind
        res_df = res_df.append(df, ignore_index=True)

#cahngre column order for plotting convenience
...
colors = [{'spk':'r', 'vm':'b'}[item] for item in res_df.kind.values]
fig = plt.figure(figsize=(10, 10))
#axes= axes.flatten()
ax = fig.add_subplot(111)
pd.plotting.scatter_matrix(res_df, ax=ax, alpha=0.3, 
                           color=colors, 
...
g = sns.pairplot(res_df.loc[res_df.kind == 'vm'], kind='reg', diag_kind='kde',
                 markers=['p'])
g.fig.suptitle('Vm')
 
g = sns.pairplot(res_df.loc[res_df.kind == 'spk'], kind='reg', diag_kind='kde',
                 markers=['2'])
g.fig.suptitle('spk')

#%% see https://blog.insightdatascience.com/data-visualization-in-python-advanced-functionality-in-seaborn-20d217f1a9a6
plt.close('all')
g = sns.pairplot(res_df, kind='reg', diag_kind='kde', hue='kind', palette='Set1')
